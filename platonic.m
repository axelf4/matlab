...
for i = 1:size(S, 1)
    ...
    if S(j, 1) == S(j, 2) || S(j, 1) == S(j, 3) || S(j, 2) == S(j, 3) || any(arrayfun(@(k) all(ismember(S(k, :), S(j, :))), 1:j-1))
        S(j, :) = [];
        offset = offset + 1;
        continue
    end
    ...
    %{
    if vecnorm(H(:, S(j, 2)) - H(:, S(j, 1))) + vecnorm(H(:, S(j, 3)) - H(:, S(j, 1))) + vecnorm(H(:, S(j, 2)) - H(:, S(j, 3))) > 6
        S(j, :) = [];
        offset = offset + 1;
        continue
    end
...
for i=1:size(S,1)
    Si=S(i,:);
    fill3(H(1,Si),H(2,Si),H(3,Si),[0.4 0.5 1], 'facealpha',0.4)
end
%}
